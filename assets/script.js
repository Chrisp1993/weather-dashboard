...
function populate5DayForecast(secondCallData) {
    ...
    for (i=1; i <= 5; i++) {
        ...
        var iconWeather = secondCallData.daily[i].weather[0].icon // weather icon
        var fahrenheitTemp = secondCallData.daily[i].temp.day // temperature
        var humidity = secondCallData.daily[i].humidity; //humidity
...
function getWeatherData(event , cityClicked) {
    ...
    // check to see if array is null and create new one again.
    ...
    fetch(  
      openWeatherApiUrl
        ...
        if (cityExist === 0) {
            ...
            // save cities searched to local storage
